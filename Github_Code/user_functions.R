...
#         taxa.output.name: all names of taxa considered by both methdos
#         z_v: the group allocation vector (n-by-1)
#         A_matrix: an n-by-p matrix with each entry being the estimated alpha_ij
#         H_matrix: an n-by-p matrix with each entry being the estimated probability that the 
#                   corresponding element in the observed count matirx (Y) is true 0
# ---------------------------------------------------------------------------------------------
# Output: a figure compareing the abundance of the given taxon between 2 patient groups
#         
# =============================================================================================
dist_compare = function(taxa.plot, taxa.output.name, z_v, A_matrix, H_matrix){
  check.nam.ind = which(taxa.output.name == taxa.plot)
  check.nam.ind_comp = which(colnames(Y_composition) == taxa.plot)
  
  data_temp = data.frame(Group = as.factor(z_v), Abundance = log(A_matrix[,check.nam.ind ]), 
                         H = H_matrix[,check.nam.ind])
  Bayes = ggplot(aes(x = Group, y = Abundance), data = subset(data_temp,H < 0.5 ) ) + 
    geom_violin(position=position_dodge(width=0.5)) + 
    stat_summary(fun.y = mean, geom = "point", size = 2, colour = "red")+
    xlab("") + ylab(expression(paste("log(", alpha["ij"], ")")))
  
  # RA #
  data_compo = data.frame(Group = as.factor(z_v), Fraction = Y_composition[,check.nam.ind_comp]+ 1e-12)
  
  compostional = 
    ggplot(aes(x = Group, y = Fraction), data = data_compo) + 
    geom_violin(width = 1,position=position_dodge(width=0.5)) + 
    stat_summary(fun.y = median, geom = "point", size = 2, colour = "blue") +
    xlab("") + ylab("compositional data") +
    scale_y_continuous(trans='log10')
  
  
  title1 = textGrob(taxa.plot, gp=gpar(fontsize = 15, fontface="bold"))
  title2 = textGrob(paste0("Kruskal-Wallis test: p-value = ", round(pval_nonpara[check.nam.ind_comp], 3), " \n ZINB-DPP: PPI = ", round(PPI[check.nam.ind], 3)),
                    gp=gpar(fontsize = 10))
  ggres = gridExtra::grid.arrange(compostional, Bayes, nrow = 1, top = title1, 
                       bottom = title2)
  return(ggres)
}
...
tree.plot = function(controln, casen, hl_size = 4, line_wd = 0.5){
  require(cowplot)
  require(ggpubr)
  require(ggtree)
  require(data.tree)
  require(microbiomeViz)
...
gamma.PPI.draw = function(gamma.PPI, tax.name.full, alpha.sig = 0.01){
  require(ggplot2)
...
#         
# =============================================================================================
CIalpha.plot = function(PPI, tax.name.full, log_r_mat, alpha.sig = 0.01, 
                          inputname.1 = "Control", inputname.2 = "Case", 
                          shift_right = 5.5,shift_ang = 30, size_y = 1, taxa.font = 1.5){
  require(ggplot2)
  cutoff = BayFDR(PPI, alpha.sig)
  taxon.sel = which(PPI > cutoff)
  taxon_name = tax.name.full[taxon.sel]
  alpha.sele = log_r_mat[,taxon.sel ]
  ...
  ggplot() + 
    ...
    guides(fill=FALSE) +
    labs(x = expression(paste("log(", alpha[j*1], "/", alpha[j*0], ")")), y = "Identified Taxa", color = "Enriched Group\n") +
    scale_color_manual(labels = c(inputname.1, inputname.2), values = c("maroon3", "royalblue2"))+
    coord_flip()
}
...
# ---------------------------------------------------------------------------------------------
# Input:  taxa_selected: name of taxa selected by a model
#         hl_size: size of the highlighted taxon on the cladogram
#         line_wd: line width of the tree branch
# ---------------------------------------------------------------------------------------------
#        
# Output: a cladogram highlighting the selected taxa
#         
# =============================================================================================
tree.compare = function(taxa_selected, hl_size = 4, line_wd = 0.5){
  require(cowplot)
  require(ggpubr)
  require(ggtree)
  require(data.tree)
  ...
  p = tree.backbone(tr, size = line_wd) # size: branch width
  # Specify node attributes to highlight
  dd <- data.frame(taxa = taxa_selected)
  if(dim(dd)[1] == 0){
    dd.node1 = NULL
  }else{
    p <- p %<+% dd
    dd.node1 <- p$data$node[p$data$label %in% taxa_selected]
  }
  p <- p +
    geom_point2(aes(subset = (node %in% dd.node1), col = "red", size = hl_size, alpha = 1)) + 
    scale_size_identity() + scale_alpha_identity() + scale_color_identity()
  plot(p)
}
